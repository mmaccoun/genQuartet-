...
import pandas as pd 
import sys
sys.path.append("/Users/mmaccoun/Desktop/IBD_xover_analysis/simulation/createSimulation/genQuartet/simChild/modules/data")
...
HG19_GENOMIC_SUM = INDIV_CHR_LENGTH["chr" + str(1)]
hg19_wholeGenome = dict()
hg19_wholeGenome["chr1"] = 249250621
for i in range(2,23):
    HG19_GENOMIC_SUM = HG19_GENOMIC_SUM + INDIV_CHR_LENGTH["chr" + str(i) ]
    #print(hg19Sum)
    hg19_wholeGenome.setdefault(str("chr")+ str(i), HG19_GENOMIC_SUM )

gaps = pd.read_table('filtered_gaps.txt',  sep = "\t", header = None)
print(gaps.head)
SUM_GAPS_ON_CHR = 0
for i in range(len(gaps.iloc[:, 0])) :
    gap_length = gaps.iloc[i, 2] - gaps.iloc[i, 1]
    SUM_GAPS_ON_CHR = SUM_GAPS_ON_CHR + gap_length